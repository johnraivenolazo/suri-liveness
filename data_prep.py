"""Data preparation script: Crop and resize face images."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--bbox_inc", type=float, default=1.5, help="Crop expansion factor")
    p.add_argument(
        "--label_dir",
...
def _process_single_image(args_bundle: Tuple[str, str, str, int, float]) -> None:
    _, full_img_path, save_path, target_size, bbox_inc = args_bundle
    ...
    try:
        ...
        x = int(x_ref * (real_w / 224))
        w = int(w_ref * (real_w / 224))
        y = int(y_ref * (real_h / 224))
        h = int(h_ref * (real_h / 224))
    except Exception:
        return

    cx = x + w // 2
    cy = y + h // 2
    side_len = int(max(w, h) * bbox_inc)
    ...
    h_crop = face_crop.shape[:2][0]
    interp = cv2.INTER_LANCZOS4 if h_crop < target_size else cv2.INTER_AREA
...
def main(argv: list[str] | None = None) -> int:
    args = parse_args() if argv is None else parse_args()  # argparse reads sys.argv
    ...
    # Filter by type code column (if present in this label format).
    ...
    for index_path, _ in all_files.iterrows():
        ...
        tasks.append((rel_path, full_img_path, save_path, args.size, args.bbox_inc))
